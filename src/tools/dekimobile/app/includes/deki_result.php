...
// should be called DreamResult but conflicts with existing class
class DekiResult
{
	...
	{
		$this->result = $result;
		if (Config::DEBUG && isset($this->result['uri']))
		{
	...
	public function handleResponse($hideStatus = array(401))
	{
		...
		$response = is_array($this->result['body']) ? print_r($this->result['body'], true) : $this->result['body'];
