...
class DekiForm 
{
		...
		if ($type == 'button') 
		{
			...
			else
			{
				...
				$html = '<button type="submit" name="'. self::BUTTON_ARRAY .'['.$name.']['. $value .']" '.
						'value="'.$value.'" '.implode(' ', $paras).'>'. htmlspecialchars($labeltext) .'</button>';
			}
		}
		else if ($type != 'textarea') 
		{
			$html = '<input type="'.$type.'" '.(!is_null($value) ? 'value="'.$value.'"': '').' '
				.($name != '' ? 'name="'.$name.'"': '').' '.implode(' ', $paras).' autocomplete="off" />';
		}
		else if ($type == 'textarea') 
		{
			$html = '<textarea name="'.$name.'" '.implode(' ', $paras).' autocomplete="off">'. $value .'</textarea>';
		}
		// add the field label
	...
	public static function multipleInput($type, $name, $data, $value = null, $params = array()) 
	{
		$Request = DekiRequest::getInstance();
		$field = '';
			...
			if (count($data) > 0) 
			{
				foreach($data as $k => $v) 
				{
					...
					$html.= '<span class="field">'.
							...
							'<label for="'.$id.'">'. htmlspecialchars($v) .'</label>'.
							'</span>';
				}
			}
		}
