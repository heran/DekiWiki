...
abstract class DekiService implements IDekiApiObject
{
	...
	{
		global $wgAdminPlug;
		$Plug = $wgAdminPlug->At('site', 'services');
		if (!is_null($typeFilter))
		{
			...
			{
				$Plug = $Plug->With('apikey', Config::$API_KEY);
			}
		}
		$Result = $Plug->Get();
	...
	static function &newFromArray(&$result)
	{
		if ($result['type'] == self::TYPE_AUTH)
		{
			$Service = new DekiAuthService(
				$result['init'],
				$result['sid'],
				isset($result['uri']) ? $result['uri']: null,
				$result['description'],
				isset($result['config']) ? $result['config']: null,
				isset($result['preferences']) ? $result['preferences']: null,
				$result['status']
			);
		}
		else
		...
		$Service->setError(isset($result['lasterror']) ? $result['lasterror']: null);
	...
	protected static function load($id)
	{
		if (!isset(self::$cache[$id]))
		{
			...
			$Plug = $wgAdminPlug->At('site', 'services', $id);
	...
	{
		global $wgAdminPlug;
		$Plug = $wgAdminPlug->At('site', 'services', $this->getId());
	...
	{
		global $wgAdminPlug;
		$Plug = $wgAdminPlug->At('site', 'services', $this->getId(), 'start');
	...
	{
		global $wgAdminPlug;
		$Plug = $wgAdminPlug->At('site', 'services', $this->getId(), 'stop');
